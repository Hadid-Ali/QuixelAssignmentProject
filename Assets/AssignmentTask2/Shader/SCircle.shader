Shader "QuixelTest/SCircle" {
	Properties{
		_ForegroundColor("Foreground Color", Color) = (1,1,1,1)
		_BackgroundColor("Background Color", Color) = (0,0,0,1)
		[HideInInspector]_ForegroundMask("Foreground Mask", 2D) = "white" {}
	...
	#pragma target 3.0
		...
		void surf(Input IN, inout SurfaceOutputStandard o) {
			...
			fixed x = (-0.5 + IN.uv_ForegroundMask.x) * (2 / _Radius);
			fixed y = (-0.5 + IN.uv_ForegroundMask.y) * (2 / _Radius);

			// Albedo comes from a texture tinted by color
			fixed radius = 1 - sqrt(x * x + y * y);
			clip(radius - _BackgroundCutoff);
			o.Albedo =  _BackgroundColor;//* _Radius;
			if (radius > _ForegroundCutoff) {
				o.Albedo = _ForegroundColor;// *_Radius;
			}
			o.Alpha = 1;
		}
		ENDCG
